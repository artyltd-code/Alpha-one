...
function render(){
  const all=stocks.map(calc).sort((a,b)=>b.value-a.value);
  ...
  const list=currentCategory==="全部"?all:all.filter(x=>x.category===currentCategory);
  ...
  </article>`).join(""):'<div class="empty">目前沒有這個分類的持股。</div>';
...
function openDialog(s=null){
  ...
  $("category").value=s?.category||"ETF";$("shares").value=s?.shares??"";$("cost").value=s?.cost??"";$("price").value=s?.price??0;$("dayChange").value=s?.dayChange??0;
...
$("stockForm").addEventListener("submit",e=>{
  ...
  const item={id:sid||id(),symbol:$("symbol").value.trim().toUpperCase(),name:$("name").value.trim(),category:$("category").value,shares:Number($("shares").value),cost:Number($("cost").value),price:Number($("price").value||0),dayChange:Number($("dayChange").value||0)};
...
$("exportBtn").addEventListener("click",()=>{
  const blob=new Blob([JSON.stringify({version:"1.2",stocks,exportedAt:new Date().toISOString()},null,2)],{type:"application/json"});
...
$("sampleBtn").addEventListener("click",()=>{
  ...
  stocks=[
    {id:id(),symbol:"00919",name:"群益台灣精選高息",category:"ETF",shares:127000,cost:21.35,price:24.10,dayChange:0.84},
    {id:id(),symbol:"0050",name:"元大台灣50",category:"ETF",shares:2000,cost:174.6,price:188.2,dayChange:1.15},
    {id:id(),symbol:"2882",name:"國泰金",category:"金融",shares:5000,cost:54.8,price:61.2,dayChange:-0.49},
    {id:id(),symbol:"3017",name:"奇鋐",category:"科技",shares:300,cost:735,price:812,dayChange:3.42}
  ];save();toast("示範資料已載入");
});
